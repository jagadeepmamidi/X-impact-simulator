...
| Deployment/storage | Production Render blueprint, per-owner credential configuration, 30-day retention, and a 1 GB persistent disk on a paid Starter service. `SQLITE_PATH` supports a separate mounted database without hiding persona packs. SQLite sidecars are ignored by Git. |
